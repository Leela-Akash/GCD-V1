...
class App {
  ...
  onResize() {
    ...
    const viewportWidth = height * this.camera.aspect;
    this.viewport = { width: viewportWidth, height };
    if (this.medias) {
      this.medias.forEach(media => media.onResize({ screen: this.screen, viewport: this.viewport }));
    }
  }
  update() {
    // Auto-rotate if enabled and no recent user interaction
    if (this.autoRotate && Date.now() - this.lastInteraction > 2000) {
...
export default function CircularGallery({
  ...
  useEffect(() => {
    const app = new App(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, autoRotate, autoRotateSpeed });
    return () => {
      app.destroy();
    };
